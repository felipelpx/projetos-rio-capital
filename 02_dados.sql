...
--
-- Os ids de origem ficam guardados em pm_projects.id_origem /
-- pm_tasks.id_origem, para se poder repetir a importação sem duplicar
-- e para conferir contra o quadro antigo.
-- ============================================================================
...
values
  ('fp_levant',(select id from public.pm_projects where id_origem='pr_fourpoints'),'Levantamento in loco + MQT','doing','alta','2026-08-03','2026-10-05','2026-10-05',40,null,1000,'2026-09-21T10:00:00.000Z'),
  ('fp_orcam',(select id from public.pm_projects where id_origem='pr_fourpoints'),'Orçamentos das construtoras','todo','media','2026-10-05','2026-11-06','2026-11-06',0,null,1000,'2026-09-21T10:01:00.000Z'),
  ('fp_analise',(select id from public.pm_projects where id_origem='pr_fourpoints'),'Análise/aprovação de orçamentos e adjudicação','todo','media','2026-11-06','2026-11-30','2026-11-30',0,null,2000,'2026-09-21T10:02:00.000Z'),
  ...
  ('br_estim',(select id from public.pm_projects where id_origem='pr_braamcamp'),'Estimativa orçamental (Proj. Nuno)','doing','media','2026-09-15','2026-10-15','2026-10-15',30,null,2000,'2026-09-21T10:07:00.000Z'),
  ...
  ('br_exec',(select id from public.pm_projects where id_origem='pr_braamcamp'),'Projeto de execução e MQT','todo','media','2026-10-30','2026-11-30','2026-11-30',0,null,5000,'2026-09-21T10:09:00.000Z'),
  ('ar_hsp',(select id from public.pm_projects where id_origem='pr_arroios'),'Documentação HSP','done','media','2026-08-03','2026-08-28','2026-08-28',100,null,1000,'2026-09-21T10:10:00.000Z'),
  ...
  ('ar_orcloja',(select id from public.pm_projects where id_origem='pr_arroios'),'Orçamento, ajuste de lojas (Edson)','done','media','2026-09-14','2026-09-17','2026-09-17',100,null,3000,'2026-09-21T10:13:00.000Z'),
  ('ar_orclimp',(select id from public.pm_projects where id_origem='pr_arroios'),'Orçamento, limpeza','review','alta','2026-09-14','2026-09-30','2026-09-22',80,'Já foram apresentados 2 orçamentos: 250 € + IVA e 650 € + IVA.',1000,'2026-09-21T10:14:00.000Z'),
  ('ar_obras',(select id from public.pm_projects where id_origem='pr_arroios'),'Obras de ajuste da loja','doing','alta','2026-09-18','2026-09-23','2026-09-23',60,null,3000,'2026-09-21T10:15:00.000Z'),
  ('ar_limpeza',(select id from public.pm_projects where id_origem='pr_arroios'),'Limpeza pós-obra','todo','alta','2026-09-24','2026-09-25','2026-09-25',0,'Previsão de custo em setembro: 300 € (aprox.).',6000,'2026-09-21T10:16:00.000Z'),
  ...
  ('ma_docs',(select id from public.pm_projects where id_origem='pr_marinha'),'Receber docs/projetos (Victor Vitorino)','waiting','alta','2026-07-13','2026-09-30','2026-09-30',60,null,6000,'2026-09-21T10:20:00.000Z'),
  ...
  ('ma_orc',(select id from public.pm_projects where id_origem='pr_marinha'),'Orçamento de obra','todo','media','2026-10-01','2026-10-30','2026-10-30',0,null,8000,'2026-09-21T10:22:00.000Z'),
  ('ma_adjud',(select id from public.pm_projects where id_origem='pr_marinha'),'Análise/aprovação e adjudicação','todo','media','2026-11-01','2026-11-30','2026-11-30',0,null,9000,'2026-09-21T10:23:00.000Z'),
  ('ma_demo',(select id from public.pm_projects where id_origem='pr_marinha'),'Demolição','todo','media','2026-12-05','2026-12-22','2026-12-22',0,null,10000,'2026-09-21T10:24:00.000Z'),
  ('ma_obra',(select id from public.pm_projects where id_origem='pr_marinha'),'Obra (18 meses)','todo','media','2027-01-05','2028-07-05','2028-07-05',0,null,11000,'2026-09-21T10:25:00.000Z'),
  ...
  ('gs_topo',(select id from public.pm_projects where id_origem='pr_sintra'),'Topografia','done','media','2026-09-10','2026-09-18','2026-09-18',100,null,4000,'2026-09-21T10:27:00.000Z'),
  ('gs_arq',(select id from public.pm_projects where id_origem='pr_sintra'),'Projeto arquitetónico (Arq. Carla)','doing','alta','2026-09-18','2026-09-30','2026-09-30',40,'Validar datas com a Arq. Carla.',6000,'2026-09-21T10:28:00.000Z'),
  ('gs_lic',(select id from public.pm_projects where id_origem='pr_sintra'),'Licença de obra','todo','media','2026-10-01','2026-11-01','2026-11-01',0,'Contacto do Henrique na CMS, caso seja necessário.',12000,'2026-09-21T10:29:00.000Z'),
  ...
  ('bf_vender6',(select id from public.pm_projects where id_origem='pr_benfica'),'Vender os 6 apartamentos que já estão legais','doing','media',null,null,null,0,null,9000,'2026-09-21T10:33:00.000Z'),
  ('bf_vender5',(select id from public.pm_projects where id_origem='pr_benfica'),'Vender as outras 5 frações por 650 mil euros','todo','media',null,null,null,0,null,14000,'2026-09-21T10:34:00.000Z'),
  ...
  ('s6q2yyd7dtepktnhhn4i',(select id from public.pm_projects where id_origem='pr_arroios'),'A','todo','media',null,null,null,0,null,15000,'2026-09-22T16:07:11.115Z')
on conflict (id_origem) do nothing;

-- Dependências ---------------------------------------------------------------
-- dias_espera = dias que têm de passar entre o fim da antecessora e o arranque.
...
-- Dependências que hoje não estão a ser respeitadas (deve devolver 0 linhas):
